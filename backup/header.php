...
<html class="no-js moscow" <?php language_attributes(); ?>>
	...
	<head>
		...
		if ( false === ( $stocks = get_transient( 'stocks_data' ) ) ) {
			...
			if($stocks["date"] < $now) {
				...
				//Текущие данные записываем в old_stocks
				...
				//Обновляем инфу, так как данные были записаны ранее 00,00 часов текущего дня
				//$url = "https://www.kitco.com/gold.londonfix.html";
				...
				//обновляем только те данные, которые пришли не пустые
				...
				echo '<script> console.log("Stocks has been updated!");</script>';

				set_transient( 'stocks_data', $stocks, 1000 * YEAR_IN_SECONDS );
				
				echo '<script> var old_stock_rub ='.$old_stocks["rub"].'; var old_stock_eur = '.$old_stocks["eur"].'; var old_stock_gold='.$old_stocks["gold"].'; var old_stock_silver='.$old_stocks["silver"].'; var old_stock_platinum='.$old_stocks["platinum"].'; var old_stock_palladium='.$old_stocks["palladium"].'; var old_stock_date = '.$old_stocks["date"].';</script>';
				echo '<script> var stock_rub ='.$stocks["rub"].'; var stock_eur = '.$stocks["eur"].'; var stock_gold='.$stocks["gold"].'; var stock_silver='.$stocks["silver"].'; var stock_platinum='.$stocks["platinum"].'; var stock_palladium='.$stocks["palladium"].'; var stock_date = '.$stocks["date"].';</script>';
			} else {
		...
		//https://developer.wordpress.org/apis/handbook/transients/   - Кэш вордпресса
        ...
        <div id="mobile_menu">
          <ul>
		   ...
		   <div class="selector top-selector" style="margin: 0 0 20px; display: flex; justify-content: center;">
            <div class="selector-title" id="mobile-popup-city"><span>Москва</span></div>
			<div id="citypopup" style="display: none;">
				<div class="inners selector">
					 <ul class="selector-dd">
						  <li class="active" onclick="location.href='https://sxematika.ru'"><span data-title="Москве">Москва</span></li>
      ...
      <div class="flex flex-pull-right">
        <div class="top-selector-wrapper">
          ...
          <div class="selector top-selector">
            <div class="selector-title"><span>Москве</span></div>
            <ul class="selector-dd">
               <li class="active" onclick="location.href='https://sxematika.ru'"><span data-title="Москве">Москва</span></li>
			  <li onclick="location.href='https://krasnodar.sxematika.ru'"><span data-title="Краснодаре">Краснодар</span></li>
			  <li onclick="location.href='https://essentuki.sxematika.ru'"><span data-title="Ессентуках">Ессентуки</span></li>
